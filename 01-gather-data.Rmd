...
determine_files_to_write <- function(files_to_download, prefix, dir_to_write, n_chunks = 1, remove = FALSE) {
  ...
  sftp_files <- all_files[grep(files_to_download, all_files[, filename]), .(filename, date)]
...
setnames(all_files, c('V1', 'V2'), c('filename', 'date'))
```
...
rna_files_to_download <- paste0('/nfs/home/students/ga89koc/ihec-incoming/RNA-Seq/.*(', paste(c(uuids_to_download[, uuid], uuids_total_mrna), collapse = '|'), ')\\.isoforms\\.results$')

determine_files_to_write(files_to_download = rna_files_to_download,prefix = 'rna', dir_to_write = rna_data_dir, n_chunks = 1)
...
determine_files_to_write(files_to_download = chip_files_to_download, prefix = 'chip', dir_to_write = chip_data_dir, n_chunks = 1)
# 
# 
...
determine_files_to_write(files_to_download = wgbs_files_to_download, prefix = 'wgbs', dir_to_write = wgbs_data_dir, n_chunks = 1)
