...
melted_psi <- melt(event_dt, measure.vars = sample_cols, id.vars = c('ID', 'Event Type', 'gene_id'), variable.name = 'uuid', value.name = 'PSI', variable.factor = FALSE)
...
aggregated_dt[melted_psi[ID %in% keep_rows_manual & !is.na(PSI)], on=.(ID, IHEC), c("uuid", "PSI"):=.(uuid, PSI)]
# remove NA PSIs
aggregated_dt <- aggregated_dt[!is.na(PSI)]

# gather the intrinsic features
cols_to_add <- c('gene_id', 'distance_TSS', 'distance_TES', 'distance_gene_start', 'distance_gene_end', 'width;upstream_other_region', 'width;event_name', 'width;downstream_other_region', 'Event Type', '3ss', '3ssdown', '5ss', '5ssup'
                 # , 'Variability'
                 )
