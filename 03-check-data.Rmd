...
tpm_all <- pca_plots(tpm_matrix[metadata[, EpiRR_no_version], ], metadata, 'TPM')
print(tpm_all$scree)
...
```{r, fig.height=7, fig.width=10}
library(umap)
umap_tpm <- umap::umap(tpm_matrix[metadata[, EpiRR_no_version], ])
...
```{r, fig.height=7, fig.width=10}
psi_all <- pca_plots(psi_table[metadata[, EpiRR_no_version], ], metadata, 'PSI')
print(psi_all$scree)
...
```{r, fig.height=7, fig.width=10}
library(umap)
umap_psi <- umap::umap(psi_table[metadata[, EpiRR_no_version], ])
