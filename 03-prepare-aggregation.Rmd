...
```{r, filter_suppa_events}
library(UpSetR)
library(grid)

min_average_PSI <- .05
max_average_PSI <- 1 - min_average_PSI
average_tpm_coverage <- 1

psi_input_dir <- "suppa_analysis/events_filtered"
# first read the samples that have been quantified
header <- readLines("suppa_analysis/tpm_expressions_filtered.tsv", n = 1)
...
keep_ids <- sapply(as_events, function(event_type) {
  suppa_psi_file <- paste0(psi_input_dir, "/event_", event_type, ".psi")
  suppa_tpm_file <- paste0(psi_input_dir, "/event_", event_type, ".tpm") # stores the summed TPM expression of all transcripts involved in the event

  event_dt <- fread(suppa_psi_file)
  tpm_dt <- fread(suppa_tpm_file)

  setnames(event_dt, "V1", "event_id")
  setnames(tpm_dt, "V1", "event_id")

  long_dt <- melt(event_dt, id.vars = "event_id", variable.name = "uuid", value.name = "psi", variable.factor = TRUE)
  long_dt[file_metadata, on = .(uuid), c("experiment_type", "epirr_id_without_version") := .(i.experiment_type, i.epirr_id_without_version)]
  
  long_tpm_dt <- melt(tpm_dt, id.vars = "event_id", variable.name = "uuid", value.name = "tpm", variable.factor = TRUE)
  long_tpm_dt[file_metadata, on = .(uuid), c("experiment_type", "epirr_id_without_version") := .(i.experiment_type, i.epirr_id_without_version)]

  average_psi_dt <-
        rbind(
          long_dt[, .(average_psi = mean(psi, na.rm = TRUE)),
            by =
              .(event_id)
          ],
          long_dt[, .(average_psi = mean(psi, na.rm = TRUE)),
            by =
              .(experiment_type, event_id)
          ],
          fill = TRUE
        )
          print(
          ggplot(average_psi_dt, aes(x = average_psi, fill = experiment_type)) +
            ...
            labs(title = sprintf("Average PSI: %s", event_type))
        )
        print(
          ggplot(
    ...
    average_tpm_dt <-
        rbind(
          long_tpm_dt[, .(average_tpm = mean(tpm, na.rm = TRUE)),
            by =
              .(event_id)
          ],
          long_tpm_dt[, .(average_tpm = mean(tpm, na.rm = TRUE)),
        ...
        print(
          ggplot(
            average_tpm_dt,
            aes(x = average_tpm + 1, color = experiment_type)
          ) +
            geom_line(aes(y = 1 - after_stat(y)), stat = "ecdf") +
            scale_x_log10() +
            geom_vline(xintercept = average_tpm_coverage + 1) +
            labs(title = sprintf("Average Read Coverage: %s with line at %d", event_type, average_tpm_coverage + 1))
        )
    average_dt <- merge(average_psi_dt, average_tpm_dt)
   
   min_tpm_cutoff <- seq(0, 1, by = 0.2)
   min_PSI_cutoffs <- seq(0, 0.05, by = 0.01)

        plot_dt <- CJ(
          average_tpm_coverage = min_tpm_cutoff,
        ...
        plot_dt[, n_pass := average_dt[
          !is.na(experiment_type) &
            average_tpm >= average_tpm_coverage &
            average_psi >= min_average_PSI &
            average_psi <= 1 - min_average_PSI,
          event_id,
          by = experiment_type
        ][duplicated(event_id), .N],
        by = .(average_tpm_coverage, min_average_PSI)
        ]

        print(ggplot(plot_dt, aes(
          x = min_average_PSI, y = n_pass,
          color = factor(average_tpm_coverage)
        )) +
          geom_line() +
          geom_point() +
          labs(
            title = sprintf("Min averages per experiment type: %s", event_type),
            x = "Minimum average PSI",
            y = "Number of events (ID) retained",
            color = "Min average tpm coverage"
          ))
  ...
  # keep only events with at least two values that are not NA
  # valid_sds <- apply(event_dt[, ..sample_cols], 1, sd, na.rm = TRUE) > 0
  # valid_sds[is.na(valid_sds)] <- FALSE
  event_dt <- event_dt[event_dt$event_id %in% keep_ids_orig]
  
  intersect_ids <- keep_ids_orig
  
  rmats_file <- sprintf("suppa_analysis/events_rmats/event_%s.psi", event_type)
  if (file.exists(rmats_file)) {
    rmats_dt <- fread(rmats_file)
    setnames(rmats_dt, "V1", "suppa_id")
    # create upsetplot of events found by both methods
    method_upset <- upset(fromList(list(suppa = intersect_ids, rmats = rmats_dt$suppa_id)))
    print(method_upset)
    grid.text(sprintf("%s", event_type), x = 0.65, y = 0.95, gp = gpar(fontsize = 20))
    intersect_ids <- intersect(keep_ids_orig, rmats_dt$suppa_id)
  }
  keep_ids_orig
}, simplify = FALSE)
```
...
```{r, load-events}
# either "filtered" or "basic", depending on the annotation used above
for (filter_suffix in c("rmats", "filtered")) {
  psi_input_dir <- paste("suppa_analysis/events", filter_suffix, sep = "_")
  # first read the samples that have been quantified
  header <- readLines(paste0("suppa_analysis/tpm_expressions_", filter_suffix, ".tsv"), n = 1)
  sample_cols <- strsplit(header, "\t", TRUE)[[1]]
  # gather per event type results
  event_res <- sapply(as_events, function(event_type) {
    # load psi values
    psi_file <- paste0(psi_input_dir, "/event_", event_type, ".psi")
    if (!file.exists(psi_file)) {
      warning(paste("File not found:", psi_file))
      return(NULL)
    }
    event_dt <- fread(psi_file)
    setnames(event_dt, "V1", "event_id")

    #TODO: remove once all rmats is processed:
    sample_cols <- intersect(sample_cols, names(event_dt))
    event_dt <- event_dt[, c("event_id", sample_cols), with = FALSE]
    
    event_dt[, table(event_id %in% keep_ids[[event_type]])]

    # keep only events with at least two values that are not NA
    valid_sds <- apply(event_dt[, ..sample_cols], 1, sd, na.rm = TRUE) > 0
    valid_sds[is.na(valid_sds)] <- FALSE
    event_dt <- event_dt[valid_sds]
  ...
  for (this_event in to_analyze) {
    ...
    pdf(file = file.path(plot_dir, paste0(this_event, "_", filter_suffix, "_event_intersections.pdf")), width = 7, height = 5, onefile = FALSE)
  ...
  manual_ids <- c() # TODO new manual ids necessary? 4019, 4020, 460:463, 310:314, 8201:8202, 1658, 1048:1049, 13896:13897)
  ...
  # write to file
  ss5_file <- sprintf("processed_data/5ss_%s.fasta", filter_suffix)
  ...
  # write to file
  ss3_file <- sprintf("processed_data/3ss_%s.fasta", filter_suffix)
  writeXStringSet(ss3, filepath = ss3_file)

  # define the ss3 and ss5 score files
  ss3_score_file <- sprintf("processed_data/3scores_%s.txt", filter_suffix)
  ss5_score_file <- sprintf("processed_data/5scores_%s.txt", filter_suffix)
  #```

  #```{bash, engine.opts="-l"}
  system(sprintf("bash -lc 'conda activate ihec-as && wget -q -O maxentscan.tar.gz http://hollywood.mit.edu/burgelab/maxent/download/fordownload.tar.gz && tar -xf maxentscan.tar.gz && mv -f fordownload maxentscan && rm maxentscan.tar.gz && cd maxentscan && perl score3.pl ../%s > ../%s && perl score5.pl ../%s > ../%s && cd .. && rm -r maxentscan'", ss3_file, ss3_score_file, ss5_file, ss5_score_file), intern = TRUE)
  #```
  ...
  # load annotation
  annotation <- rtracklayer::import(sprintf("suppa_analysis/gencode.v29.%s.gtf", filter_suffix))

  # load suppa results of inclusion and exclusion transcripts
  ioes <- rbindlist(lapply(to_analyze, function(event_type) fread(paste0(psi_input_dir, "/gencode.v29.", filter_suffix, ".events_", event_type, "_strict.ioe"))))
  ...
  rtracklayer::export(aggregateOver, sprintf("processed_data/aggregateOver_%s.bed", filter_suffix))
  #```


  #```{r, save-workspace}
  save.image(sprintf("processed_data/aggregating_%s.rda", filter_suffix))
}
```
