...
write_chunked_files <- function(commands, n_chunks, name, folder) {
  chunked_commands <- ifelse(n_chunks == 1, list(commands), split(commands, cut(seq_along(commands), n_chunks, labels = FALSE)))
...
```{r}
# make sure you ran "sshfs DATA-545_qmanz@sftp.bcgsc.ca:/IHEC/incoming/ /nfs/home/students/ga89koc/ihec-incoming/"
files_to_download <- paste0('.*(', paste(rna_samples_w_chip[, sort(unique(epirr_id_wo_version))], collapse = '|'), ').*.(fc.signal.bigwig|pval0.01.500K.bfilt.narrowPeak.gz)$')
...
```{r}
# make sure you ran "sshfs DATA-545_qmanz@sftp.bcgsc.ca:/IHEC/incoming/ /nfs/home/students/ga89koc/ihec-incoming/"
files_to_download <- paste0('.*(', paste(rna_samples_w_wgbs[, sort(unique(epirr_id_wo_version))], collapse = '|'), ').*.gembs_(neg|pos).bw$')
