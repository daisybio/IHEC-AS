...
for (this_event in to_analyze) {
  ...
  not_chrY <- this_dt[seqnames != 'chrY', which = TRUE]
  ...
  upset <- upset(fromList(list('all'=this_dt[, seq.int(.N)],'no overlap other'=non_overlapping_other, 'no overlap self'=non_overlapping_self, 'single gene'=single_genes_only, 'not chrY' = not_chrY)), sets.bar.color = "#56B4E9", order.by = "freq")
  print(upset)
...
not_chrY <- event_dt[seqnames != 'chrY', which = TRUE]

keep_rows <- intersect(intersect(single_genes_only, non_overlapping_other), not_chrY)
