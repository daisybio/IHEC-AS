...
```{r}
psi_input_dir <- "suppa_analysis/events/filtered_annotation/TPM1"
event_res <- sapply(as_events, function(event) {
	# load psi values
  event_dt <- fread(paste0(psi_input_dir, '/event_', event, '.psi'))
  ...
  list(event_dt = event_dt[, c('ID', 'gene_id', 'seqnames', 'strand', ..samples_to_consider)], event_gr = event_gr, upstream_gr = upstream_gr, downstream_gr = downstream_gr, single_genes_only = single_genes_only, non_overlapping = non_overlapping)
}, simplify = FALSE)
```
...
var_events <- sapply(names(event_res), function(event) {
  ...
  events_to_consider <- intersect(this_event[['non_overlapping']], which(apply(event_mat, 1, function(r) sum(is.na(r)) < (length(r) - 1))))
...
single_genes_only <- integer()
non_overlapping <- integer()
for (event_name in names(event_res)) {
  ...
  single_genes_only <- c(single_genes_only, event_res[[event_name]]$single_genes_only + max_ID)
  non_overlapping <- c(non_overlapping, event_res[[event_name]]$non_overlapping + max_ID)
...
max_cor_cols <- keep_cols(aggregated_dt, aggregation)
aggregated_dt_intermediate <- aggregated_dt[, ..max_cor_cols]
...
for (event_name in event_dt[, unique(event_name)]) {
  var_events_type <- var_events[[event_type]]
  for (var_threshold in names(var_events_type)) {
    cor_dt_event_var <- cor_dt_intermediate[event == event_name & `variance quartile` == var_threshold]
    ...
    pheatmap(t(cor_mat[samples_with_epi, ])[, rownames(annotation_df)[order(annotation_df$annotation)]], color = colorRampPalette(c("red", "white", "blue"))(length(breaks)), breaks = breaks,
             annotation_col = annotation_df, cluster_cols = FALSE, main = paste(event_name, ': PSI Variance Quantile:', var_threshold))
  }
}
